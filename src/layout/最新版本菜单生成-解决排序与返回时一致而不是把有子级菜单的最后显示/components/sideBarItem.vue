<template>
  <!-- 1. 顶级菜单且不需要子菜单 -->
  <el-menu-item
    v-if="item.enableFlag && item.meta?.notShowChildren"
    class="show-tip"
  ...
  >
    ...
    <template #title>
      ...
      <img
        ...
        v-if="item.icon !== null"
      />
      <span class="menu-leval1" v-show="!isFold">{{ item.menuName }}</span>
      <!-- 弹出层 -->
      <div
        class="side-pop"
        v-if="item.frontCode == navCode && isFold"
              ...
              <el-menu-item
                v-if="
                  ...
                  (!('children' in li) ||
                    ...
                    li.children?.filter((m) => m.enableFlag).length == 0)
                "
                :index="li.href"
      ...
      <template v-for="(li, idx) in item.children">
        ...
        <template
          v-if="
            !('children' in li) ||
            li.children.length == 0 ||
            li.children.filter((s) => s.enableFlag).length == 0
          "
        >
          <el-menu-item
            v-if="li.enableFlag"
